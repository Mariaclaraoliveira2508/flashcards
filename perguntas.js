criaCartao(
'Química',
'Qual o modelo atômico de Thomson?',
'Modelo pudim de passas.'
)

```
criaCartao(
'Química',
'Qual o símbolo químico do ouro?',
'Au.'
)
criaCartao(
'Química',
'Qual a fórmula química do sal de cozinha?',
'NaCl.'
)
criaCartao(
'Química',
'O que é uma mistura homogênea?',
'Uma mistura com apenas uma fase visível.'
)
criaCartao(
'Química',
'Qual a passagem do estado sólido para o líquido?',
'Fusão.'
)
criaCartao(
'Química',
'Quem propôs o modelo atômico planetário?',
'Rutherford.'
)
criaCartao(
'Química',
'O que é um elemento químico?',
'Uma substância formada por apenas um tipo de átomo.'
)
criaCartao(
'Química',
'Qual o gás mais abundante na atmosfera da Terra?',
'Nitrogênio.'
)
criaCartao(
'Química',
'O que mede a escala de pH?',
'A acidez ou basicidade de uma solução.'
)
```